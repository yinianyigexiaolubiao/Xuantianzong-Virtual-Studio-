...
def render_qc(scene):
    ...
    for image in QC_OUTPUT.glob("qc_*.png"):
        image.unlink()
    scene.frame_set(1)
    ...
    clear_pngs(PEAK_QC_OUTPUT)
    peak_cameras = sorted(obj.name for obj in bpy.data.objects if obj.name.startswith("XTZ_CAM_XTPEAK_QC_"))
    if len(peak_cameras) < 6:
        raise RuntimeError(f"Expected >=6 Xuantian Peak QC cameras, found {len(peak_cameras)}")
    ...
    reveal_rock = bpy.data.objects.get("XTZ_V02_PASS_ROCK_REVEAL_WEST")
    if reveal_rock:
    ...
    for index, (frame, label) in enumerate(((121, "05s"), (145, "06s"), (169, "07s"), (193, "08s")), 5):
        scene.frame_set(frame)
        render_still(scene, "XTZ_V02_CAM_DJI_28MM", SWORD_QC_OUTPUT / f"sword_qc_{index:02d}_dji_{label}.png")

    clear_pngs(DJI_QC_OUTPUT)
    keyframes = (
        (1, "00s"), (49, "02s"), (73, "03s"), (109, "04p5s"),
        (145, "06s"), (181, "07p5s"), (205, "08p5s"), (229, "09p5s"),
        (253, "10p5s"), (277, "11p5s"), (301, "12p5s"),
        (325, "13p5s"), (360, "15s"),
    )
